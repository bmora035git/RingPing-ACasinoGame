...
public class ringPing_v2 {
    public static void main(String args[ ]) {
        ...
        // String variables for the reels

        String cherryWheel;
        String bellWheel;
        String sevenWheel;

        boolean minorJackpot = false;
        boolean majorJackpot = false;
        

        // Start with 9 spins, and declare the weighted tokens for 3 of a kind

        int spins = 9;

        // Declare the weighted tokens for 3 of a kind
        int weightedTokenThreeCherries = 3;
        int weightedTokenThreeBells = 5;
        int weightedTokenThreeSevens = 7;

        // Declare the variable for the spin, and the total tokens won
        boolean spin = false;
        int tokensWon = 0;

        // Declare an array for the symbols on the reels

        String[] symbols = {"symbol_1", "symbol_2", "symbol_3"};

        // Do-While loop for the game, which will continue until the spins are exhausted
        // The player will be prompted to spin the reels, and the outcome will be displayed.  
        // The player can win tokens for 3 of a kind, and can win a bonus spin for drawing a cherry.
        
        do  {

            // Display the number of spins remaining, and prompt the player to spin the reels
            System.out.println("Spins Remaining: " + spins + "\nPress Enter to Spin the Reels...");
            
            // Wait for the player to press enter to spin the reels
            while (spin == scanner.nextLine().isEmpty())
                if (spin != true) {
                    System.out.println("Please Press Enter to Spin the Reels...");
            ...
            // Call the reel methods to get the outcome of each reel, and store them in the symbols array
            symbols[0] = reel_1();
            symbols[1] = reel_2();
            symbols[2] = reel_3();

        
            // Display the outcome of each reel
            ...
            // If symbols are three of a kind, then a weighted token is won.  If there are 2 of a kind, then no token is won, and 
            // if a cherry is in the virtual reel, you get a bonus spin.
            if(symbols[0].equals("Cherry") && symbols[1].equals("Cherry") && symbols[2].equals("Cherry")) {
                ...
                System.out.println("No win this spin. Better luck next time!");
                majorJackpot = false;
                minorJackpot = false;
            ...
            display(minorJackpot, majorJackpot);
            
        } while (spins > 0);
    }

    // Methods for the reels, which will generate a random number between 1 and 100, and return 
    // the corresponding symbol based on the weight
    ...
    // Method to display the outcome of the reels, and the total tokens won
    public static void display(boolean minorJackpot, boolean majorJackpot) {
        if (minorJackpot) {
            System.out.println("Congratulations! You won the mini jackpot for 3 bells!");
            
        }
        if (majorJackpot) {
            System.out.println("Congratulations! You won the major jackpot for 3 sevens!");
